...
export default function AdminDashboard() {
  ...
  const RoleBadge = ({ role }: { role: string }) => <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${role.toLowerCase() === "creator" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}>{role}</span>;
  const StatusBadge = ({ status }: { status: string }) => <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${status.toLowerCase() === "active" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}>{status}</span>;
